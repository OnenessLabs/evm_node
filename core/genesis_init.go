...
func (env *genesisInit) callContract(contract common.Address, method string, args ...interface{}) ([]byte, error) {
	...
	if rules := env.genesis.Config.Rules(env.header.Number, true, env.header.Time); rules.IsBerlin {
	...
	// Run evm call
	ret, _, err := evm.Call(vm.AccountRef(msg.From), *msg.To, msg.Data, msg.GasLimit, uint256.MustFromBig(msg.Value))
...
func (env *genesisInit) initStaking() error {
	...
	env.state.SetBalance(system.StakingContract, uint256.MustFromBig(contract.Balance))
...
func (env *genesisInit) initGenesisLock() error {
	...
	env.state.SetBalance(system.GenesisLockContract, uint256.MustFromBig(contract.Balance))
