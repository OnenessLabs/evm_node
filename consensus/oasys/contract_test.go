...
func (p *testBlockchainAPI) Call2(ctx context.Context, args ethapi.TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash, overrides *ethapi.StateOverride) (hexutil.Bytes, error) {
	if p.count == nil {
...
func makeEnv(wallet accounts.Wallet, account accounts.Account) (*testEnv, error) {
	...
	// Generate StateDB
	//statedb := tests.MakePreState(db, genspec.Alloc, false, "hash")
	...
	return &testEnv{engine, chain, nil}, nil
}
