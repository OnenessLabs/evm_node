...
type chainContext struct {
	...
	oasys consensus.ConsensusEngine
}

func (c chainContext) Engine() consensus.ConsensusEngine {
	return c.oasys
}
...
type blockchainAPI interface {
	Call2(ctx context.Context, args ethapi.TransactionArgs, blockNrOrHash *rpc.BlockNumberOrHash, overrides *ethapi.StateOverride) (hexutil.Bytes, error)
}
...
// Call2 the `StakeManager.getValidators` method.
func callGetValidators(ethAPI blockchainAPI, hash common.Hash, epoch uint64) (*nextValidators, error) {
	...
	for {
		...
		blockNumberOrHash := rpc.BlockNumberOrHashWithHash(hash, false)
		rbytes, err := ethAPI.Call2(
			ctx,
...
// Call2 the `CandidateValidatorManager.getHighStakes` method.
...
// Call2 the `StakeManager.getValidatorOwners` method.
...
// Call2 the `StakeManager.getTotalRewards` method.
...
// Call2 the `Environment.nextValue` method.
...
func (c *Oasys) applyTransaction(
	...
	state.PrepareOasys(expectedTx.Hash(), len(*txs))
