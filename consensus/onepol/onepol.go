...
// OnePOL proof-of-stake protocol constants.
var (
...
// OnePOL is the proof-of-stake consensus engine
type OnePOL struct {
	chainConfig *params.ChainConfig  // Chain config
	config      *params.OnePOLConfig // Consensus engine configuration parameters
...
// New creates a OnePOL proof-of-stake consensus engine with the initial
// signers set to the ones provided by the user.
func New(chainConfig *params.ChainConfig, config *params.OnePOLConfig, db ethdb.Database, ethAPI *ethapi.BlockChainAPI) *OnePOL {
...
func (c *OnePOL) Author(header *types.Header) (common.Address, error) {
	return header.Coinbase, nil
}

// VerifyHeader checks whether a header conforms to the consensus rules.
func (c *OnePOL) VerifyHeader(chain consensus.ChainHeaderReader, header *types.Header) error {
...
func (c *OnePOL) VerifyHeaders(chain consensus.ChainHeaderReader, headers []*types.Header) (chan<- struct{}, <-chan error) {
...
func (c *OnePOL) verifyHeader(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (c *OnePOL) verifyCascadingFields(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header, env *environmentValue) error {
...
func (c *OnePOL) snapshot(chain consensus.ChainHeaderReader, number uint64, hash common.Hash, parents []*types.Header) (*Snapshot, error) {
...
func (c *OnePOL) VerifyUncles(chain consensus.ChainReader, block *types.Block) error {
...
// from.
func (c *OnePOL) verifySeal(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header, scheduler *scheduler) error {
...
func (c *OnePOL) Prepare(chain consensus.ChainHeaderReader, header *types.Header) error {
	number := header.Number.Uint64()
...
// rewards given.
func (c *OnePOL) Finalize(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs *[]*types.Transaction,
	...
	cx := chainContext{Chain: chain, onepol: c}
	if number == 1 {
...
func (c *OnePOL) FinalizeAndAssemble(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB,
...
// with.
func (c *OnePOL) Authorize(signer common.Address, signFn SignerFn, txSignFn TxSignerFn) {
	c.lock.Lock()
	defer c.lock.Unlock()
...
func (c *OnePOL) Seal(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	header := block.Header()
	...
	// Sign all the things!
	sighash, err := signFn(accounts.Account{Address: validator}, accounts.MimetypeOnePOL, OnePOLRLP(header))
	if err != nil {
		return err
	}
...
func (c *OnePOL) CalcDifficulty(chain consensus.ChainHeaderReader, time uint64, parent *types.Header) *big.Int {
	...
	return scheduler.difficulty(number, c.signer, c.chainConfig.IsForkedOnePOLExtendDifficulty(parent.Number))
}

// SealHash returns the hash of a block prior to it being sealed.
func (c *OnePOL) SealHash(header *types.Header) common.Hash {
	return SealHash(header)
}

// Close implements consensus.Engine. It's a noop for onepol as there are no background threads.
func (c *OnePOL) Close() error {
	return nil
}

// APIs implements consensus.Engine, returning the user facing RPC API to allow
// controlling the signer voting.
func (c *OnePOL) APIs(chain consensus.ChainHeaderReader) []rpc.API {
	return []rpc.API{{
		Namespace: "onepol",
		Version:   "1.0",
		Service:   &API{chain: chain, onepol: c},
		Public:    false,
	}}
}

// Converting the validator list for the extra header field.
func (c *OnePOL) getExtraHeaderValueInEpoch(number *big.Int, validators []common.Address) []byte {
	cpy := make([]common.Address, len(validators))
	copy(cpy, validators)

	forked := c.chainConfig.IsForkedOnePOLPublication(number)
	if !forked {
...
func (c *OnePOL) verifyExtraHeaderLengthInEpoch(number *big.Int, length int) error {
	if c.chainConfig.IsForkedOnePOLPublication(number) {
...
func (c *OnePOL) verifyExtraHeaderValueInEpoch(number *big.Int, actual []byte, validators []common.Address) error {
...
// OnePOLRLP returns the rlp bytes which needs to be signed for the proof-of-stake
...
func OnePOLRLP(header *types.Header) []byte {
	b := new(bytes.Buffer)
...
func (c *OnePOL) addBalanceToStakeManager(state *state.StateDB, hash common.Hash, number uint64, env *environmentValue) error {
...
func (c *OnePOL) environment(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) (*environmentValue, error) {
	number := header.Number.Uint64()
	if number < c.config.Epoch {
...
func (c *OnePOL) scheduler(chain consensus.ChainHeaderReader, header *types.Header,
