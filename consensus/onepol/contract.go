...
var (
	//go:embed onepol-genesis-contract-cfb3cd0/artifacts/contracts/Environment.sol/Environment.json
	//go:embed onepol-genesis-contract-cfb3cd0/artifacts/contracts/StakeManager.sol/StakeManager.json
	//go:embed onepol-genesis-contract-6037082/artifacts/contracts/CandidateValidatorManager.sol/CandidateValidatorManager.json
	artifacts embed.FS
	...
	environment = &genesisContract{
		...
		artifact: &artifact{
			path: filepath.FromSlash("onepol-genesis-contract-cfb3cd0/artifacts/contracts/Environment.sol/Environment.json"),
		},
	}
	stakeManager = &genesisContract{
		address: common.HexToAddress(stakeManagerAddress),
		artifact: &artifact{
			path: filepath.FromSlash("onepol-genesis-contract-cfb3cd0/artifacts/contracts/StakeManager.sol/StakeManager.json"),
		},
	}
	...
	candidateManager = &builtinContract{
		...
		artifact: &artifact{
			path: filepath.FromSlash("onepol-genesis-contract-6037082/artifacts/contracts/CandidateValidatorManager.sol/CandidateValidatorManager.json"),
		},
	}
)
...
func getInitialEnvironment(config *params.OnePOLConfig) *environmentValue {
	return &environmentValue{
...
func (c *OnePOL) IsSystemTransaction(tx *types.Transaction, header *types.Header) (bool, error) {
	// deploy transaction
	if tx.To() == nil {
		return false, nil
	}
...
func (c *OnePOL) initializeSystemContracts(
	state *state.StateDB,
	header *types.Header,
...
func getNextValidators(
	...
	if config.IsForkedOnePOLPublication(new(big.Int).SetUint64(block)) {
...
func callGetValidators(ethAPI blockchainAPI, hash common.Hash, epoch uint64) (*nextValidators, error) {
	...
	for {
		...
		blockNumberOrHashWithHash := rpc.BlockNumberOrHashWithHash(hash, false)
...
func (c *OnePOL) applyTransaction(
	msg callmsg,
	state *state.StateDB,
	header *types.Header,
	cx core.ChainContext,
	...
	state.PrepareOnePOL(expectedTx.Hash(), len(*txs))
	...
	*usedGas += gasUsed
	...
	receipt.Logs = state.GetLogs(expectedTx.Hash(), uint64(expectedTx.Time().Unix()), common.Hash{})
