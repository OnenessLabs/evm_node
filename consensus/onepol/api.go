...
func (api *API) GetSnapshot(number *rpc.BlockNumber) (*Snapshot, error) {
	...
	return api.onepol.snapshot(api.chain, header.Number.Uint64(), header.Hash(), nil)
}
...
func (api *API) GetSigners(number *rpc.BlockNumber) ([]common.Address, error) {
	...
	snap, err := api.onepol.snapshot(api.chain, header.Number.Uint64(), header.Hash(), nil)
	if err != nil {
		return nil, err
	}
...
func (api *API) Proposals() map[common.Address]bool {
	api.onepol.lock.RLock()
	defer api.onepol.lock.RUnlock()

	proposals := make(map[common.Address]bool)
	for address, auth := range api.onepol.proposals {
...
func (api *API) Propose(address common.Address, auth bool) {
	api.onepol.lock.Lock()
	defer api.onepol.lock.Unlock()

	api.onepol.proposals[address] = auth
}
...
func (api *API) Discard(address common.Address) {
	...
	delete(api.onepol.proposals, address)
}
...
// 		sealer, err := api.onepol.Author(h)
// 		if err != nil {
// 			return nil, err
// 		}
// 		signStatus[sealer]++
...
// GetSigner returns the signer for a specific onepol block.
...
func (api *API) GetSigner(rlpOrBlockNr *blockNumberOrHashOrRLP) (common.Address, error) {
	if len(rlpOrBlockNr.RLP) == 0 {
		...
		return api.onepol.Author(header)
	}
	block := new(types.Block)
	if err := rlp.DecodeBytes(rlpOrBlockNr.RLP, block); err == nil {
		return api.onepol.Author(block.Header())
	}
