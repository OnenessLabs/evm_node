...
func TestGetNextValidators(t *testing.T) {
	...
	config := &params.ChainConfig{ChainID: big.NewInt(999999), OnePOL: &params.OnePOLConfig{}}
...
func makeEnv(wallet accounts.Wallet, account accounts.Account) (*testEnv, error) {
	...
	engine := New(chainConfig, chainConfig.OnePOL, db, nil)
