...
func (c *Chaos) VerifyHeader(chain consensus.ChainHeaderReader, header *types.Header) error {
...
func (c *Chaos) VerifyHeaders(chain consensus.ChainHeaderReader, headers []*types.Header) (chan<- struct{}, <-chan error) {
...
func (c *Chaos) FinalizeAndAssemble(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB,
	txs []*types.Transaction, uncles []*types.Header, receipts []*types.Receipt, withdrawals []*types.Withdrawal) (b *types.Block, rs []*types.Receipt, err error) {
	defer func() {
		if err != nil {
